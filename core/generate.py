...
def generate_draft_answer(question: str, retrieved_context: list) -> str:
    ...
    This function now expects the full list of search results from Qdrant.
    """
    ...
    for i, item in enumerate(retrieved_context):
        # CORRECTED: Look for "text_content" instead of "answer"
        answer_piece = item.payload.get("text_content", "").strip()
        source_file = item.payload.get("source_file", "N/A")

        if answer_piece:
            # IMPROVEMENT: Add the source file and score for better context
